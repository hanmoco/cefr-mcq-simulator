...
def confusion_matrix(
    ...
    model.eval()
    counts = {"mO_lO": 0, "mX_lX": 0, "mX_lX_same": 0, "mO_lX": 0, "mX_lO": 0, "N": 0}
    ...
    for batch in dataloader:
        ...
        counts["mX_lX_same"] += ((~model_correct & ~learner_correct) & (model_top1 == learner_top1)).sum().item()
...
def main(argv=None):
    ...
    train_loader = DataLoader(
        MCQDataset(data_train, tokenizer, args.max_length),
        batch_size=args.batch_size,
        shuffle=True,
        collate_fn=lambda batch: collate(batch, pad_token_id=tokenizer.pad_token_id),
    )
    ...
    for epoch in range(args.epochs):
        train_kl = train_epoch(model, train_loader, optimizer, choice_ids, device)
        val_kl = np.mean([
            evaluate(model, val_loaders[lv], choice_ids, device)["KL"] for lv in LEVELS
        ])

        print(f"[Epoch {epoch+1}] Train KL={train_kl:.4f} | Val KL={val_kl:.4f}")

        if val_kl < best_val_kl:
            best_val_kl = val_kl
            patience = 0
            best_adaptor_dir = os.path.join(args.out_dir, "best_lora_adaptor")
            os.makedirs(best_adaptor_dir, exist_ok=True)
            model.save_pretrained(best_adaptor_dir)
        else:
            patience += 1
    ...
    model = PeftModel.from_pretrained(base_model, os.path.join(args.out_dir, "best_lora_adaptor"))
    model.eval()
